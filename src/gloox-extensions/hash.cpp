...
Hash::Hash(std::string algo, const std::string& hash)
	: StanzaExtension(EXT_HASHES), m_algo(algo), m_hash(hash),
	m_valid(!m_algo.empty())
{
}
...
{
	if (!tag || tag->findAttribute("xmlns") != XMLNS_HASHES_2 ||
	    !tag->hasAttribute("algo"))
		return;
	...
	if (tag->name() == "hash") {
		m_hash = tag->cdata();
		m_valid = !m_algo.empty() && !m_hash.empty();
...
Tag* Hash::tag() const
{
	...
	if (!m_hash.empty()) {
		Tag* tag = new Tag("hash", XMLNS, XMLNS_HASHES_2);
		tag->addAttribute("algo", m_algo);
		tag->addCData(m_hash);
		return tag;
	}

	Tag* tag = new Tag("hash-used", XMLNS, XMLNS_HASHES_2);
	tag->addAttribute("algo", m_algo);
	return tag;
}
...
{
	static const std::string filter = "/*/*[@xmlns='" + XMLNS_HASHES_2 + "']";
	return filter;
}
